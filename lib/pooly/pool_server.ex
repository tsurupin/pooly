defmodule Pooly.PoolServer do
  ...
  def init([{:max_overflow, max_overflow}|rest], state) do
    init(rest, %{state | max_oveflow: max_overflow})
  end
    ...
    case :ets.lookup(monitors, pid) do
      ...
      _ ->
        case Enum.member?(workers, pid) do
          true ->
            remaiing_workers = workers |> Enum.reject(fn(p) -> p == pid end)
            new_state = %{state | workers: [new_worker(worker_sup) | remaiing_workers]}
            {:noreply, new_state}
          false ->
            {:noreply, state}
        end
